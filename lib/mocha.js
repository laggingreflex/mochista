...
const { parent, child } = require('./utils/fork');
...
if (module.parent) {
  ...
  child({
    run: (argv, files) => new Promise((resolve, reject) => {
      ...
      if (mocha.addRequiresForSubprocess) {
        mocha.addRequiresForSubprocess(argv.require)
      } else {
...
function load(config = {}) {
  ...
  let Mocha;
  if (config.parallel) {
    Mocha = require('mocha-parallel-tests').default;
  } else {
    Mocha = require('mocha');
  }
  ...
  const ifn = c => f => c && f(c);
  const opt = new Proxy({}, { get: (_, o) => typeof o === 'string' && (opts[o] || config[o]) });
